...
function isoWindow(tab: Tab): { from: string; to: string } {
  const now = new Date();
  const from = now.toISOString().slice(0, 10) + 'T00:00:00.000Z';
  const to = new Date(now.getTime() + (tab === 'today' ? 24 : 14) * 24 * 60 * 60 * 1000)
    ...
    <Screen toast={msg} onDismissToast={() => setMsg(null)}>
      ...
      {loading ? (
        ...
        <View style={s.empty}>
          ...
          <Text muted>No services scheduled.</Text>
        </View>
      ) : (
        services.map(svc => (
          <Card key={svc.id}>
            ...
            >
              <View style={s.serviceRow}>
                ...
                <View style={s.serviceBody}>
                  ...
                  <Text variant="body">
                    {formatDateLabel(svc.starts_at)} · {formatTime(svc.starts_at)}
